...
final_pred %>% count(LossofIncome_cover,interaction != 0) # To check #

# final_pred %>%
#   mutate(error = if_else(LossofIncome_cover==0,
#                          (grossincurred_prop + grossincurred_lossofinc - 
#                             (prop_freq_NB * prop_sev +
#                             bi_freq_NB * bi_sev +
#                             interaction))^2,
#                          (grossincurred_prop - prop_freq_NB * prop_sev)^2)) %>%
...
VIP_plot(freqmodelbi_NB,title="LoI Frequency")
...
anova(freqmodelprop_NB,test="Chisq") #anova test
